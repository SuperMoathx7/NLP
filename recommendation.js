...
async function findPhoneRecommendations(text) {
    ...
    await showPhones(final_phones, maxPoint, criteria, text)
}

async function showPhones(final_phones, maxPoint, criteria, text) {
    if (counter != 0) {
        removeLastMessage()
    }
    counter += 5
    const responseDiv = document.getElementById('response');
    const comments = await commentOnResults(final_phones.slice(0, counter), text)
    let htmlResponse = `<p class="mb-3 ${cur_lang == "ar" ? "pl-4" : "pr-4"}">${comments[0]}</p>`;
    let ind = 1
    ...
    if (counter < final_phones.length) {
        ...
        document.getElementById('showMoreBtn')?.addEventListener('click', async () => await showPhones(final_phones, maxPoint, criteria, text));
    } else {
        addMessage(htmlResponse, false)
        // responseDiv.innerHTML = htmlResponse
    }
...
async function commentOnResults(phones, text) {
    const prompt = 
    `
    Hi, I gave you this prompt ${text}, and according to that I got some information about some phones, I have ${phones.length} phones, and I want you to give me only ${phones.length + 1} lines after each other, each paragraph in a line, separate them with empty lines, like:
    the first line (40-50 words) would a comment from you given to the user talking about the requiremtns he asked and how the results match what he asked or not, then each line would be (20-30 words) a comment for each phone in orded and how this phone will be good choice for him.
    ${cur_lang == "ar" ? "give me the texts in arabic" : ""}
    Just give me the lines without anything else and make the text to talk with the user who asked the query not to me (something like "The phone you're asking about with these properties...."), here is the phones:
    ${phones.map(phone => JSON.stringify(phone)).join("\n")}
    `
    let prompt_response = await getResponse(prompt)
    prompt_response = prompt_response.split("\n")
    let result = []
    for (let i = 0; i < prompt_response.length; i += 2) {
        result.push(prompt_response[i].trim())
    }
    return result
}
